...
RSpec.describe "Parser.executeにS式を渡すと配列になる" do 
  ...
  it "(print (join (join 'think' 'different') 'this' 'is' (join 'apple' 'world!')))" do
    ...
    expect(Parser.execute(code: code)).to eq([:print, [:join, [:join, "think", "different"], "this", "is", [:join, "apple", "world!"]]])
  end
  ...
  it 'Lispファイルをパースできる' do
    code = File.open('spec/fixtures/test.lisp').read
    expect(Parser.execute(code: code)).to eq([:print, [:join, "hello", "world"]])
  end
end
